...
function execute(cmd) {
    ...
    getSession().then(function (token) {
        switch (cmd.type) {
            ...
            case 'appDetail':
                return appDetail(token, cmd.appId);
            case 'appRemove':
                return appRemove(token, cmd.appId);
        }
    }, function (err) {
        console.log(''
            + chalk.yellow('[ERROR] ')
...
function appDetail(token, appId) {
    var url = URL_APP + '/' + appId;
    sendRequest('get', url, null, token)
...
function appRemove(token, appId) {
    var url = URL_APP + '/' + appId;
    sendRequest('delete', url, null, token)
        .then(function () {
            console.log('Successfully removed ' + chalk.cyan(appId));
        }, function (error) {
